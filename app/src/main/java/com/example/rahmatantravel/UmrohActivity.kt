...
class UmrohActivity : AppCompatActivity() {
    ...
    private lateinit var recyclerPaketUmroh: RecyclerView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val dialog = Dialog(this@UmrohActivity)
        ...
        recyclerPaketUmroh = findViewById(R.id.recyclerView)
        imageView = findViewById(R.id.imageView)

        // Sembunyikan imageView di awal
        imageView.visibility = View.GONE

        RetrofitClient.instance.getPaketByMenu("Umrah").enqueue(object : Callback<APIResponse> {
                ...
                if (response.isSuccessful){
                    ...
                    if (apiResponse != null) {
                        ...
                        if (data.isNullOrEmpty()) {
                            ...
                            recyclerPaketUmroh.visibility = View.GONE
                        } else {
                            // Jika data tidak kosong, tampilkan recyclerView dan sembunyikan imageView
                            imageView.visibility = View.GONE
                            recyclerPaketUmroh.visibility = View.VISIBLE
                            ...
                            adapter.onItemClick = { keberangkatanItem, paketItem, hotelItem ->
                                val intent = Intent(this@UmrohActivity, DetailPaket::class.java)
                                intent.putExtra("keberangkatan_id", keberangkatanItem.id.toString())
                                intent.putExtra("paket_id", paketItem.id_paket.toString())
                                startActivity(intent)
                            }
                            val layoutManager1 = LinearLayoutManager(this@UmrohActivity, LinearLayoutManager.VERTICAL, false)
                            recyclerPaketUmroh.layoutManager = layoutManager1
                            recyclerPaketUmroh.adapter = adapter
